...
b$.module("gadgets.pubsub", function() {
    ...
    var EventBus = Class.extend(function() {
        ...
        publish: function(channelName, oMessage, flush, eventType) {
            ...
            oMessage = oMessage || {}
            eventType = eventType || "";
            if(eventType !== "SYSTEM" && eventType !== "RENDERER") {
    ...
    /**
     * Subscribes a widget to a specific renderer channel message channel.
     *
     * @param {String} channelName The name of the channel
     * @param {Function} callback A function that will be called with the channel messages
     */
    function subscribeMessage(channelName, callback) {
        mainBus.subscribe(channelName+"-RENDERER", callback);
    }
