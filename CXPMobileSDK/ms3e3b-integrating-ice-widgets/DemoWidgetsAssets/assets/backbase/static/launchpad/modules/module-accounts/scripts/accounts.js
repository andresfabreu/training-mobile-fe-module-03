		...
		this.$get = function($http, $q) {
			...
			*/
			...
			API.prototype.load = function(justRefresh) {
				...
				$http.get(self.getAttribute(CFG_ACCOUNTS_ENDPOINT)).then(function(response) {
					self.refreshAccounts(response.data, justRefresh);
					deferred.resolve(self.accounts);
				});

				// Provide backward compatibility, as load fn previously didn't return
				// an actual promise.
