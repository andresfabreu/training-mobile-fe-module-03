...
{
    // Check if the device is capable of sending an email
...
{
    // Check if the device is capable of sending an email, return an error if not
    ...
    // Compose mail controller
    MFMailComposeViewController *mailVC = [[MFMailComposeViewController alloc] init];
    mailVC.mailComposeDelegate = self; // This will cause the mailComposeController:didFinishWithResult:error: method to be executed (defined below)
...
- (void)callPhoneNumber:(NSString*)phoneNumber
{
    ...
    BOOL result = [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[NSString stringWithFormat:@"tel:%@", phoneNumber]]];
    if(result) {
...
- (void)mailComposeController:(MFMailComposeViewController *)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError *)error
{
    ...
    if(result == MFMailComposeResultFailed) {
        NSString *errorMessage = error.localizedDescription ? error.localizedDescription : @"Email failed to sent because of an unknown error";
