...
/**
 * This method is creating the initial view controller. Currently it's copying the splash screen to allow windows to preload.
 */
...
- (void)setupBackbaseCXP {
    ...
    if([CXP isDeviceJailbroken]) {
        [[[UIAlertView alloc] initWithTitle:@"Device is jailbroken" message:@"For your own safety we don't allow users with jailbroken devices to use this application." delegate:nil cancelButtonTitle:nil otherButtonTitles:nil] show];
        return;
    }
    ...
    if (error) {
        [CXP logError:self message:[NSString stringWithFormat:@"Unable to read configuration due error: %@", error.localizedDescription ? error.localizedDescription : @"Unknown error"]];
    }
    
    // Register the contact feature that is used in the About page
    [CXP registerFeature:[ContactFeature new] error:&error];
    if (error) {
        [CXP logError:self  message:[NSString stringWithFormat:@"Unable register contact feature due error: %@", error.localizedDescription ? error.localizedDescription : @"Unknown error"]];
    }
...
//    [CXP model:self order:@[kModelSourceFile]];
...
/**
 * This method is executed when all items scheduled for preloaded, are preloaded. It's mainly used to initialize the user interace and hide the splash screen after the user interface is initialized.
 */
...
/**
 * This method is executed when the library detects a navigation request from a widget. The notification contains information about the origin, target and relation of the origin and target. This information is used to determine what en how to show a page.
 */
...
/**
 * This method is executed when the model is loaded. It's creating a reference to the model so we can use it at a later stage.
 */
...
- (void)modelDidFailLoadWithError:(NSError *)error {
    ...
    [[[UIAlertView alloc] initWithTitle:@"Error while loading model" message:@"The app model couldn't be loaded. This is most likely because of a missing or incorrect implemented model. Please inform the organisation. Restart or reinstall the application to continue using it." delegate:nil cancelButtonTitle:nil otherButtonTitles:nil] show];
}

#pragma mark - SecurityViolationDelegate

/**
 * This method is executed when a (security) policy violation has occurred. It can be used to block the usage of the app.
 */
- (void)securityDidReceiveViolation:(NSError*)error {
    
    // Show a non-closable error indicating that something bad happened
    [[[UIAlertView alloc] initWithTitle:@"Security policy violation" message:@"The app's security policy has been violated. Please inform the organisation. Restart or reinstall the application to continue using it." delegate:nil cancelButtonTitle:nil otherButtonTitles:nil] show];
}
