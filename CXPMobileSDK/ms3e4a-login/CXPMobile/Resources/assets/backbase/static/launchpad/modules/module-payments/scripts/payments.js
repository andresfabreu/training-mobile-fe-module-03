        ...
        this.$get = function($q, $resource, lpCoreUtils) {
            ...
            function API() {
                ...
                Rest.prototype.createModel = function() {
                    return new Rest({
                        id: generateUUID() // to be refactored
                    });
                };
                ...
                Rest.prototype.load = function(accounts) {
                    ...
                    var self = this;
                    var data = [];
                    var params = { // This is to not use default paging from payments Rest
                        f: 0,   // paging: from
                        l: 100, // paging: size
                        ids: accounts.join(',')
                    };
                    Rest.getUnsubmitted(params, function(resp) {
                        data = resp;
                        self.pendingOrdersGroups = self.preprocessPendingOrdersGroups(data);
                        deferred.resolve(self.pendingOrdersGroups);
                    }, function(error) {
                    });
                    return deferred.promise;
                };
