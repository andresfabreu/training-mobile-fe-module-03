define(function(require, exports, module) {
    ...
    // @ngInject
    exports.ContactsController = function($scope, $timeout, lpWidget, ContactsModel, AccountsModel, TransactionsModel, $filter, i18nUtils, customerId, lpCoreBus, lpCoreUtils) {
        var ALERT_TIMEOUT = 3000;

        var bus = lpCoreBus;
        var widget = lpWidget;

        $scope.partialsDir = lpCoreUtils.getWidgetBaseUrl(widget) + '/partials';
        ...
        var initializeTransactions = function() {
            ...
            if(!$scope.transactionsModel) {
                $scope.transactionsModel = TransactionsModel.getInstance({
                    transactionsEndpoint: '$(contextPath)/services/rest/v1/current-accounts/$(accountId)/transactions',
                    pageSize: 5
                });
            }
        ...
        var contactSelected = function(contact) {
            ...
            if(accountsModelCollection && accountsModelCollection.length > 0) {
                $scope.transactionsModel.setFilters({
                    contact: contact.account
                });
                $scope.transactionsModel.loadTransactions($scope.defaultAccount);
            }
        ...
        $scope.$watch('contactsModel.moduleState', function(value) {
            if(value) {
                $scope.contactsModel.template = $scope.partialsDir + '/' + value + '.html';
            }
        });
    ...
    exports.ContactsPaymentController = function($scope, lpPayments, lpCoreBus, lpCoreUtils) {
        ...
        $scope.resetPaymentOrder = function(contact) {
            ...
            $scope.paymentOrder.uuid = util.generateUUID();
