define(function (require, exports, module) {
    ...
    // TODO: Remove all jQuery references :)
    var $ = window.jQuery;
    ...
    // @ngInject
    exports.lpCategorySelect = function($templateCache, $timeout, lpCoreBus) {
            ...
            link: function (scope, element, attrs) {
                ...
                var cloneCategoryList = function() {
                    ...
                    for (var i = 0; i < scope.model.categories.length; i++) {
                        scope.transactionCategories.push(angular.copy(scope.model.categories[i]));
                    }
                };
                cloneCategoryList();
                ...
                var goToCategoryStep = function(stepNumber) {
                    if (scope.smallLayout) {
                        $timeout(function() {
                            $('body').animate({
                                scrollTop: $('#transaction-' + scope.transaction.id + (stepNumber > 1 ? '-category-step-' + stepNumber : '')).offset().top - 5 - scope.offsetTopCorrection
                            }, 500);
                        }, 0);
                    }
                };
                ...
                scope.selectColor = function(color) {
                    scope.model.create(scope.categoryFilter, color).then(function(response) {
                        scope.categoryFilter = '';
                        scope.showColorPicker = false;
                        cloneCategoryList();
                        scope.markNewCategory(response.data.id);
                ...
                scope.deleteCategory = function(event, category) {
                    ...
                    if (category.type === 'NORMAL') {
                        scope.model.del(category.id).success(function() {
